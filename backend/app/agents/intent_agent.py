...
class IntentAgent:
    ...
    async def classify_intent(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            log_exception(logger, e, context=f"Intent classification failed: {user_input}")
    ...
    async def validate_intent(self, state: AgentState) -> AgentState:
        ...
        try:
            if hasattr(self.openrouter, "ainvoke"):
                from langchain_core.messages import HumanMessage, SystemMessage
                resp = await self.openrouter.ainvoke([
                    SystemMessage(content=INTENT_VALIDATOR_PROMPT),
            ...
            log_exception(logger, e, context="Intent validation failed")
            state["clarification_question"] = "I’m having trouble understanding your request. Could you rephrase it?"
            state["error"] = str(e)
            return state

def create_intent_node(openrouter_client: Any, user_repo: UserRepository, audit_repo: AuditRepository):
