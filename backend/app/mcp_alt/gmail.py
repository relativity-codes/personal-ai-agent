...
async def get_headers(user_id: Optional[str] = None) -> Dict[str, str]:
    try:
        token = await get_token(user_id)
        if token:
            return {"Authorization": f"Bearer {token}", "credentials": "include"} if token else {}
        else:
            return await get_token(user_id, refresh=True)
    except Exception as e:
...
async def list_threads(query: str = "is:unread", max_results: int = 10, user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=30.0, headers=await get_headers(user_id)) as client:
